    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Range"))
        {
            if (FindObjectOfType<PlayerController>().dashing)
            {
                ...
                // GetComponent<LifeManager>().TakeDamage(100);
                StartCoroutine(Wait());
            }
        }
    }
    private void FixedUpdate()
    {
        ...
        if (!gameObject.CompareTag("dead"))
        {
            ...
            if (player)
            {
                ...
                MeleeRange = (Vector3.Distance(player.transform.position, transform.position) < MeleeDist);
                ...
                if (Vector2.Distance(player.transform.position, transform.position) < range && !MeleeRange)
                {
